...
private:
    ...
    std::vector<std::vector<std::tuple<std::string, std::string, NodeIndex>>> *assignments_;
...
**/
NodeExpander::NodeExpander(Graph<> *_origin_graph, Graph<> *graph, config::Configuration * conf)
{
    original_graph_= _origin_graph;
    search_graph_ = graph;
    config = conf;
...
void NodeExpander::expandNode(Node *node)
{
    ...
    assignments_ = assignment_generator->generateAgentActionAssignments(*original_graph_, nodes);
    ...
    for (auto &cur_assignments : *assignments_)
    {
        ...
        for (auto &agent_action_assignment : cur_assignments)
        {
            ...
            Node *action_ptr = original_graph_->getNode(std::get<2>(agent_action_assignment));
...
Node *NodeExpander::createInteraction(Node *destination_or, std::string i_name, double i_cost)
{
    ...
    auto or_prime = original_graph_->insertNode(tdata);
    ...
    auto inter_action = original_graph_->insertNode(idata);
